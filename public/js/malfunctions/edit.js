    ...
    $('.malfunction-type-select').on('change', function () {
        if (this.value == 'B') {
            ...
            if(paragraph_type == "severe") {
                $('.risk').text(Lang['High']); $('.risk').data('value', 'High');
    ...
    */
    ...
    $('#calculate').on('click', function (e) {
        ...
        $.each($('.malfunction-scoring__item'), function (index, paragraph_elem) {
            ...
            var paragraph = $(paragraph_elem),
                ...
                finding = paragraph.find('.malfunction-finding-select').val(),
            ...
            $.each(paragraph.find('.summernote-textarea'), function (_index, element) {
                if (!$(element).hasClass('disabled')) {
                    ...
                    // var current_finding_id = $(element).attr("id").split("-");
                    // current_finding_id = current_finding_id[current_finding_id.length - 1];
                    
                    malfunction_principal_html += '<p>' + text + '</p>';
                    paragraph_malfunction_text += '<p>' + number + ' ' + name + '</p>' + $(element).val();
                }
            });
                ...
                `<div class="malfunction-principal__item" data-category-id="` + category_id + `" data-id="` + paragraph_id + `">
                    ...
                    <div class="malfunction-scoring__photos ` + ($.inArray(type, ["S", "B"]) === -1 ? "disabled" : "") + `">
                        ...
                        <label class="malfunction-scoring__add-photo" for="add-image-` + category_id + `-` + paragraph_id + `">
                            <input id="add-image-` + category_id + `-` + paragraph_id + `" class="malfunction-upload-image" type="file" ` + ($.inArray(type, ["S", "B"]) === -1 ? "disabled" : "") + `>
                        </label>
                    </div>
                </div>`;
            }
        ...
        if (!is_error) {
            ...
            for (category_id in categories_data) {
                var category_total = categories_data[category_id]['score'] * $('#category-score-' + category_id).data('category-value');
                ...
                if (categories_data[category_id]['in_list']) {
                    list_text += '<p><b><u>' + categories_data[category_id]['name'] + '</u></b></p>';
            ...
            $.each(malfunction_types, function(index, type) {
                switch ($(type).val()) {
                case 'S':
                    paragraph_total += 50; break;
                case 'B':
                    paragraph_total += 0; break;
                default:
                    paragraph_total += 100; break;
                }
            });
    ...
    $('#download_pdf').click(function() {
        ...
        html2pdf(document.documentElement, {
            margin:       1,
            filename:     'malfunction_scoring_'+ (new Date().getTime()) + '.pdf',
            ...
            callback: function(pdf, filename) {
                ...
                html2pdf(document.documentElement, {
                    margin:       1,
                    filename:     'malfunction_other_'+ (new Date().getTime()) + '.pdf',
                    image:        { type: 'jpeg', quality: 0.98 },
                    html2canvas:  { dpi: 192, letterRendering: true },
                    jsPDF:        { unit: 'pt', format: [width, height], orientation: 'portrait' }
                });
...
function updateChart(result) {
    ...
    chart.options.axisY.interval = max ? max / 6 : undefined;

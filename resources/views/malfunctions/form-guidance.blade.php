            ...
            <div class="card guidances {{$user_role}}">
                ...
                <div class="card-body">
                    <form id="guidance-form" style="width:100%">
                        ...
                        <input type="hidden" name="data[site]" value="{{ isset($guidance['site']) ? $guidance['site'] : '' }}"/>
                        <input type="hidden" name="data[subsite]" value="{{ isset($guidance['subsite']) ? $guidance['subsite'] : '' }}"/>
                        <input type="hidden" name="data[date]" value="{{ isset($guidance['date']) ? $guidance['date'] : '' }}"/>
                        <input type="hidden" name="data[time_]" value="{{ isset($guidance['time_']) ? $guidance['time_'] : '' }}"/>
                        <input type="hidden" name="data[employee_id]" @if ($user_role == 'admin' || $user_role == 'employee') value="{{$user_id}}" @else value="{{isset($guidance['employee_id']) ? $guidance['employee_id'] : ''}}" @endif/>
                        <input type="hidden" name="data[employee_name]" @if (($user_role == 'admin' || $user_role == 'employee') && !isset($guidance['employee_name'])) value="{{$user_name}}" @else value="{{isset($guidance['employee_name']) ? $guidance['employee_name'] : ''}}" @endif/>
                        <input type="hidden" name="data[admin_name]" value="{{isset($guidance['status']) && isset($guidance['status']['stage']) && $guidance['status']['stage'] == 'publish' ? $guidance['admin_name'] : ''}}"/>
                        <input type="hidden" name="data[status][stage]"  @if (($user_role == 'admin' || $user_role == 'employee') && (!isset($guidance['status']) || !isset($guidance['status']['stage']) || $guidance['status']['stage'] == 'draft')) value="draft" @else value="{{isset($guidance['status']) && isset($guidance['status']['stage']) ? $guidance['status']['stage'] : ''}}" @endif/>
                        ...
                        <div class="row guidance-total-summary">
                            <div class="col-md-7">
                                ...
                                <select {{ (isset($guidance['site']) && $guidance['site']) ? '' : 'disabled' }} name="subsite_" id="subsite">
                                    ...
                                    <ul style="{{App::getLocale() == 'he' ? 'padding-right: 10px !important' : 'padding-left: 10px !important'}}">
                                        <li>
                                            ...
                                            <input type="text" id="resp" class="edit-field" readonly placeholder=""
                                                    size="15" name="data[company_representative]" value="{{ isset($guidance['company_representative']) ? $guidance['company_representative'] : '' }}">
                                        </li>
                                    </ul>
                                </div>
                            </div>
                            <div class="col-md-5">
                                <ul class="employe">
                                    <li>{{__('Employee name')}}: <span>@if (($user_role == 'admin' || $user_role == 'employee') && !isset($guidance['employee_name'])) {{ $user_name }} @else {{isset($guidance['employee_name']) ? $guidance['employee_name'] : ""}} @endif</span></li>
                                    <li>{{__('Admin name')}}: <span>{{isset($guidance['admin_name']) ? $guidance['admin_name'] : ""}}</span></li>
                                    <li>{{__('Date')}}: <input id="datepicker" class="edit-field" data-date-format="mm/dd/yyyy" value="{{isset($guidance['date']) ? $guidance['date'] : ""}}"></li>
                                    <li>{{__('Time')}}: <input type="text" id="time" class="edit-field" size="4" maxlength="5" pattern="([01]?[0-9]|2[0-3]):[0-5][0-9]" onchange="updateTime(this);" value="{{isset($guidance['time_']) ? $guidance['time_'] : ""}}"></li>
                                    <li>{{__('Contract representative')}}: <input type="text" size="15" class="contractor_representative edit-field" name="data[contractor_representative]" placeholder="" value="{{ isset($guidance['contractor_representative']) ? $guidance['contractor_representative'] : '' }}" /></li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div class="col-md-12 guidance-general">
                            ...
                            <textarea name="data[guidance-general]" id="guidance-general-textarea" class="summernote-textarea">
                        ...
                        <div class="col-md-12 guidance-themes">
                            ...
                            <textarea name="data[guidance-themes]" id="guidance-themes-textarea" class="summernote-textarea">
                        ...
                        <div class="col-md-12 guidance-principal">
                                ...
                                @if (isset($guidance['guidance_principal_detail']))
                                    @foreach ($guidance['guidance_principal_detail'] as $key => $principal)
                                        ...
                                        <div class="guidance-principal-item" data-id="{{$principal["id"]}}">
                                            ...
                                            <textarea name="data[guidance_principal_detail][{{$principal["id"]}}][value]" class="summernote-textarea">
                                                {{ $principal["value"] }}
                                            </textarea>

                                            <input type="hidden" name="data[guidance_principal_detail][{{$principal["id"]}}][id]" value="{{$principal["id"]}}" />

                                            <div class="guidance-photos">
                                                @if (isset($guidance['photo'][$principal["id"]]))
                                                    <input type="hidden" name="data[photo][{{$principal["id"]}}][]" value=""/>
                                                    @foreach ($guidance['photo'][$principal["id"]] as $photo)
                                                        ...
                                                        <div class="guidance-principal-photo-item">
                                                            ...
                                                            <input type="hidden" name="data[photo][{{$principal["id"]}}][]" value="{{$photo}}"/>
                                                        </div>
                                                    @endforeach
                                                @endif
                                                <div class='upload-photo-wrapper'>
                                                    <label class="guidance-add-photo" for="add-image-{{ $principal["id"] }}">
                                                        <i class="fa fa-upload"></i>
                                                        <input id="add-image-{{ $principal["id"] }}" class="guidance-upload-image" type="file">
                                                    </label>
                                                </div>
                                                <div class='cb'></div>
                                            </div>
                            ...
                            <a href='javascript:void(0);' id='add-principal-guidance' class='btn-blue'>{{__('Add more')}}</a>
                        </div>

                        <div class="col-md-12 guidance-summary">
                            <div><h4 class="guidance-heading">4. {{__('Summary & Recommendations')}}</h4></div>
                            <textarea name="data[guidance-summary]" id="guidance-summary-textarea" class="summernote-textarea">
                            ...
                            <div id="guidance-uploaded-documents">
                                @isset($guidance['guidance-uploads'])
                                    ...
                                    <div class="guidance-uploads-item">
                                        ...
                                        <input type="hidden" name="data[guidance-uploads][]" value="{{$upload}}"/>
                                    </div>
                                    @endforeach
                                @endisset
                            </div>

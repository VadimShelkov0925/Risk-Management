            ...
            <div class="card malfunctions {{$user_role}}">
                ...
                <div class="card-header">
                    @if ((!isset($malfunction->data['status']) || !isset($malfunction->data['status']['stage']) || $malfunction->data['status']['stage'] != 'publish') && ($user_role == 'admin' || $user_role == 'employee'))
                        <div class="btn-group {{app()->getLocale() == 'he' ? 'right' : 'left'}}">
                            <a class='btn-action' onclick="deleteMalfunction();" style="color:black;" href="javascript:void(0);">
                                <i class="fa fa-trash"></i>
                            </a>
                            <a class='btn-action' style="color:black;" href="{{ action('MalfunctionController@edit', $malfunction->id) }}">
                ...
                <div class="card-body row total-summary">
                    <div class="col-md-4">
                        <h5>{{__('To')}}</h5>
                        <ul class="company">
                            <li><span>{{ (isset($malfunction->data['site']) ? $malfunction->data['site'] : '') . (isset($malfunction->data['subsite']) ? (" / " . $malfunction->data['subsite']) : '') }}</span></li>
                            <li><span>{{__('Company representative')}}: {{ isset($malfunction->data['company_representative']) ? $malfunction->data['company_representative'] : '' }}</span></li>
                        </ul>
                    </div>
                    <div class="col-md-4">
                        <ul class="employe">
                            ...
                            <li>{{__('Date')}}: {{ isset($malfunction->data['date']) ? $malfunction->data['date'] : '--/--/----' }}</li>
                ...
                <div class="card-body details">
                                                ...
                                                <table class="table table-striped" border="1" style="margin-bottom: 0px;">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td style="padding: 0px; text-align: center;">
                                                            <table class="table" style="background: transparent; margin-bottom: 0px;">
                                                                <tr style="background: transparent;">
                                                                    <td style="border-top: 0px; {{App::getLocale() == 'he' ? 'border-left: 1px solid grey;' : 'border-right: 1px solid grey;'}}"
                                                                        width="20%">{{__('TYPE_F')}}
                                                                    </td>
                                                                    <td style="border-top: 0px; {{App::getLocale() == 'he' ? 'border-left: 1px solid grey;' : 'border-right: 1px solid grey;'}}"
                                                                        width="20%">{{__('TYPE_S')}}
                                                                    ...
                                                                    <td style="border-top: 0px;" width="20%">{{__('TYPE_C')}}</td>
                                                                </tr>
                                                            </table>
                                                        </td>
                                                        <td style="padding: 0px;">
                                                            <table class="table" style="background: transparent; margin-bottom: 0px;">
                                                                <tr style="background: transparent;">
                                                                    <td style="border-top: 0px; {{App::getLocale() == 'he' ? 'border-left: 1px solid black;' : 'border-right: 1px solid black;'}}" width="65%">{{__('Relatively')}}</td>
                                                                    <td style="border-top: 0px;" width="35%">{{__('Total')}}</td>
                                                                </tr>
                                                            </table>
                                                        </td>
                                                    </tr>
                                                    ...
                                                    @if (isset($malfunction->data['paragraphs']) && isset($malfunction->data['paragraphs'][$category['id']]))
                                                        @foreach($malfunction->data['paragraphs'][$category['id']] as $paragraph_key => $paragraph)
                                                            <tr>
                                                                <td>{{ ($category_key+1) .'.'. ($paragraph_key+1) }} {{$paragraph['name']}}</td>
                                                                <td style="padding: 0px; text-align: center;">
                                                                    <table class="table" style="background: transparent; margin-bottom: 0px;">
                                                                        <tr style="background: transparent;">
                            ...
                            <ul class="malfunctions-principal">
                                @foreach($malfunction->data['malfunction_principal_detail'] as $principal)
                                    <li class="malfunctions-item" data-malfunction-id="<?php echo $malfunction_id ?>" data-category-id="<?php echo $principal["category_id"] ?>" data-principal-id="<?php echo $principal["id"] ?>">
                                        <?php echo (isset($principal["value"]) ? $principal["value"] : "") ?>
                                        ...
                                        <input class="comment-editor" data-principal-id="<?php echo $principal["id"] ?>"></input>                                    </li>
                                @endforeach
                            </ul>
                        @endif
                    </div>
                    ...
                    <div class="malfunctions-item other">
                        <h5><b>7. {{__('Summary & Recommendations')}}</b></h5>
                    ...
                    <div class="malfunctions-item other">
                        <h5>{{__('Sincerely')}},</h5>
                        <h5>{{isset($malfunction->data['employee_name']) ? $malfunction->data['employee_name'] : ""}}</h5>
                    </div>
                </div>

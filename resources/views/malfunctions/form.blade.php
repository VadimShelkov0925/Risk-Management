                ...
                <div class="card malfunctions {{$user_role}}">
                    <div class="card-header">
                        ...
                        <div class="dropdown-wrapper">
                            <div class="dropdown">
                                <label class="dropdown-toggle" data-toggle="dropdown"><i class="fa fa-upload"></i></label>
                                <ul class="dropdown-menu">
                                    <li><a href="javascript:void(0)" id='upload_from_computer'>{{__('Upload from computer')}}</a></li>
                                    <li><a href="javascript:void(0);" onclick="uploadFromSignsForms()">{{__('Upload from Signs&forms')}}</a></li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <!-- <div class="btn-group">
                            <a href="javascript:void(0);" class="btn-action" id="duplicate_malfunction" title="duplicate malfunction"><img src="{{url('/')}}/img/frontend/action/copy.png" width=25 height=25></a>
                    ...
                    <div class="card-body">
                        <form id="malfunction-form" style="width:100%;">
                            ...
                            <input type="hidden" id="calc-input-total" name="data[calculate][total]" value="{{ isset($malfunction['calculate']['total']) && strpos($malfunction['calculate']['total'], '%') !== false ? $malfunction['calculate']['total'] : '' }}">
                            <input type="hidden" name="data[site]" value="{{ isset($malfunction['site']) ? $malfunction['site'] : '' }}"/>
                            <input type="hidden" name="data[subsite]" value="{{ isset($malfunction['subsite']) ? $malfunction['subsite'] : '' }}"/>
                            <input type="hidden" name="data[date]" value="{{ isset($malfunction['date']) ? $malfunction['date'] : '' }}"/>
                            <input type="hidden" name="data[time_]" value="{{ isset($malfunction['time_']) ? $malfunction['time_'] : '' }}"/>
                            <input type="hidden" name="data[service_level]" value="{{ isset($malfunction['service_level']) ? $malfunction['service_level'] : '' }}"/>
                            <input type="hidden" name="data[risk_level]" value="{{ isset($risk['level']) ? $risk['level'] : '' }}"/>
                            <input type="hidden" name="data[employee_id]" @if ($user_role == 'admin' || $user_role == 'employee') value="{{$user_id}}" @else value="{{isset($malfunction['employee_id']) ? $malfunction['employee_id'] : ''}}" @endif/>
                            <input type="hidden" name="data[employee_name]" @if (($user_role == 'admin' || $user_role == 'employee') && !isset($malfunction['employee_name'])) value="{{$user_name}}" @else value="{{isset($malfunction['employee_name']) ? $malfunction['employee_name'] : ''}}" @endif/>
                            <input type="hidden" name="data[admin_name]" value="{{isset($malfunction['status']) && isset($malfunction['status']['stage']) && $malfunction['status']['stage'] == 'publish' ? $malfunction['admin_name'] : ''}}"/>
                            <input type="hidden" name="data[gastronomy_score]" value="{{ isset($malfunction['gastronomy_score']) && strpos($malfunction['gastronomy_score'], '%') !== false ? $malfunction['gastronomy_score'] : '' }}"/>
                            <input type="hidden" name="data[status][stage]" @if (($user_role == 'admin' || $user_role == 'employee') && (!isset($malfunction['status']) || !isset($malfunction['status']['stage']) || $malfunction['status']['stage'] == 'draft')) value="draft" @else value="{{isset($malfunction['status']) && isset($malfunction['status']['stage']) ? $malfunction['status']['stage'] : ''}}" @endif/>
                            ...
                            <div class="row malfunction-total-summary">
                                <div class="col-md-4">
                                    ...
                                    <select {{ (isset($malfunction['site']) && $malfunction['site']) ? '' : 'disabled' }} name="subsite_" id="subsite">
                                    ...
                                    <div class="col-md-12 mt-4">
                                        <ul style="{{App::getLocale() == 'he' ? 'padding-right: 10px !important' : 'padding-left: 10px !important'}}">
                                            <li>
                                                ...
                                                <input type="text" id="resp" class="edit-field" readonly placeholder="" size="15" name="data[company_representative]" value="{{ isset($malfunction['company_representative']) ? $malfunction['company_representative'] : '' }}">
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <ul class="employe">
                                        <li>{{__('Employee name')}}: <span>@if (($user_role == 'admin' || $user_role == 'employee') && !isset($malfunction['employee_name'])) {{ $user_name }} @else {{isset($malfunction['employee_name']) ? $malfunction['employee_name'] : ""}} @endif</span></li>
                                        <li>{{__('Admin name')}}: <span>{{isset($malfunction['admin_name']) ? $malfunction['admin_name'] : ""}}</span></li>
                                        <li>{{__('Date')}}: <input id="datepicker" class="edit-field" data-date-format="mm/dd/yyyy" value="{{isset($malfunction['date']) ? $malfunction['date'] : ""}}"></li>
                                        <li>{{__('Time')}}: <input type="text" id="time" class="edit-field" size="4" maxlength="5" pattern="([01]?[0-9]|2[0-3]):[0-5][0-9]" onchange="updateTime(this);" value="{{isset($malfunction['time_']) ? $malfunction['time_'] : ""}}"></li>
                                        <li>{{__('Contract representative')}}: <input type="text" size="15" class="contractor_representative edit-field" name="data[contractor_representative]" placeholder="" value="{{ isset($malfunction['contractor_representative']) ? $malfunction['contractor_representative'] : '' }}" /></li>
                                    </ul>
                                </div>
                                <div class="col-md-4">
                                    <ul class="totals">
                                        ...
                                        <li>{{__('Risk level')}}: <span class="risk">
                                        ...
                                        <li>{{__('Gastronomy score')}}: <span id="gastronome">{{ isset($malfunction['gastronomy_score']) && strpos($malfunction['gastronomy_score'], '%') !== false ? $malfunction['gastronomy_score'] : '---' }}</span></li>
                                        <li>{{__('Service level')}}:
                            ...
                            <div class="col-md-12 malfunction-scoring">
                                ...
                                @foreach ($categories as $key_category => $category)
                                    <input type="hidden" name="data[categories][{{ $key_category }}][id]" value="{{ $category->id }}">
                                    <input type="hidden" name="data[categories][{{ $key_category }}][name]" value="{{ $category->name }}">
                                    <input type="hidden" name="data[categories][{{ $key_category }}][score]" value="{{ $category->score }}">
                                    @if (count($category->paragraphs))
                                        ...
                                        <table class="malfunction-scoring__table" data-category-id="{{ $category->id }}">
                                            <thead>
                                            <tr>
                                            ...
                                            @foreach ($category->paragraphs as $key_paragraph => $paragraph)
                                                ...
                                                <input type="hidden" name="data[paragraphs][{{$category->id}}][{{$key_paragraph}}][id]" value="{{ $paragraph->id }}">
                                                <input type="hidden" name="data[paragraphs][{{$category->id}}][{{$key_paragraph}}][name]" value="{{ $paragraph->name }}">
                                                <input type="hidden" name="data[paragraphs][{{$category->id}}][{{$key_paragraph}}][score]" value="{{ $paragraph->score }}">
                                                ...
                                                <tr class="malfunction-scoring__item"
                                                    ...
                                                    <td class="malfunction-scoring__paragraph">
                                                        <span class="malfunction-scoring__paragraph-number">{{ $key_category+1 }}.{{ $key_paragraph + 1 }}</span>
                                                        <span class="malfunction-scoring__paragraph-name">{{ $paragraph->name }}</span>
                                                    </td>

                                                    <td class="malfunction-scoring__type">
                                                        <select
                                                            name="data[malfunction_type][{{ $category->id }}][{{ $paragraph->id }}]"
                                                            class="malfunction-type-select">
                                                            <option style="display: none;" disabled
                                                                    {{ !isset($malfunction['malfunction_type'][$category->id][$paragraph->id]) ? 'selected' : '' }}
                                                                    value="">{{__('F/S/B/N/C')}}
                                                            </option>
                                                            <option
                                                    ...
                                                    <td class="malfunction-scoring__risk {{ !isset($malfunction['malfunction_finding'][$category->id][$paragraph->id]) ? 'hasnt_active' : ''}}">
                                                        @foreach ($paragraph->frr as $frr)
                                                            <textarea
                                                                ...
                                                                @else
                                                                    <p class='frr_finding'>{{ $frr->finding }}</p><p class='frr_risk'>{{ $frr->risk }}</p><p class="frr_repair">{{ $frr->repair }}</p>
                                                                @endif
                                                            </textarea>
                                                        @endforeach
                                                    </td>
                                            ...
                                            @endforeach
                                            <tr>
                                                <td colspan="7" style="{{app()->getLocale() == 'he'? 'text-align: left;' : 'text-align: right;'}}"><b>{{__('Category score')}}:</b>
                                                    <span id="category-score-{{ $category->id }}"
                                                        ...
                                                        @if (isset($malfunction['calculate'][$category->id]['value']) && strpos($malfunction['calculate'][$category->id]['value'], '%') !== false)
                                                            {{ $malfunction['calculate'][$category->id]['value'] }}
                                                        @else
                                                            ---
                                                        @endif
                                                    </span>
                                ...
                                <div class="malfunction-scoring__total-score">
                                    <div class="malfunction-scoring__total-info">
                                        <b>{{__('Total score')}}</b>
                                        <span style="width: auto; {{app()->getLocale() == 'he'? 'text-align: left;' : 'text-align: right;'}}">
                                    ...
                                    <div style="{{app()->getLocale() == 'he'? 'text-align: left;' : 'text-align: right;'}}"><p id="malfunction-calc-error-msg" style="margin-bottom: 0px;
                                        color: red;"></p><a href="" class="malfunction-scoring__calculate" id="calculate">{{__('Calculate')}}</a></div>
                                </div>
                            </div>
                            <div class="col-md-12 malfunction-general malfunction-other hidden">
                                <div><h4 class="malfunction-heading">2. {{__('General')}}</h4></div>
                                <textarea name="data[malfunction-general]" id="malfunction-general-textarea" class="summernote-textarea">
                            ...
                            <div class="col-md-12 malfunction-repaired malfunction-other hidden">
                                ...
                                <textarea name="data[malfunction-repaired]" id="malfunction-repaired-textarea" class="summernote-textarea">
                                ...
                                <div id="malfunction-principal__items">
                                    ...
                                    @if (isset($malfunction['malfunction_principal_detail']))
                                        @foreach ($malfunction['malfunction_principal_detail'] as $key => $principal)
                                            @if (isset($principal["category_id"]) && isset($principal["id"]) && isset($principal["value"]))
                                                <div class="malfunction-principal__item" data-category-id="{{$principal["category_id"]}}" data-id="{{$principal["id"]}}">
                                                <div class="malfunction-sort"></div>
                                                <textarea name="data[malfunction_principal_detail][{{$key}}][value]" class="summernote-textarea">
                                                    {{ $principal["value"] }}
                                                </textarea>

                                                <input type="hidden" name="data[malfunction_principal_detail][{{$key}}][category_id]" value="{{$principal["category_id"]}}" />
                                                <input type="hidden" name="data[malfunction_principal_detail][{{$key}}][id]" value="{{$principal["id"]}}" />
                                                ...
                                                <div class="malfunction-scoring__photos {{ isset($malfunction['malfunction_type'][$principal["category_id"]][$principal["id"]]) && in_array($malfunction['malfunction_type'][$principal["category_id"]][$principal["id"]], ['S', 'B']) ? '' : 'disabled' }}" >
                                                    @if (isset($malfunction['photo'][$principal["category_id"]][$principal["id"]]))
                                                        <input type="hidden" name="data[photo][{{$principal["category_id"]}}][{{$principal["id"]}}][]" value=""/>
                                                        @foreach ($malfunction['photo'][$principal["category_id"]][$principal["id"]] as $photo)
                                                            ...
                                                            <div class="malfunction-principal__photo-item">
                                                                ...
                                                                <input type="hidden" name="data[photo][{{$principal["category_id"]}}][{{$principal["id"]}}][]" value="{{$photo}}"/>
                                                            </div>
                                                        @endforeach
                                                    @endif
                                                    <div class='upload-photo-wrapper'>
                                                        <label class="malfunction-scoring__add-photo" for="add-image-{{ $principal["category_id"] }}-{{ $principal["id"] }}">
                                                            <i class="fa fa-upload"></i>
                                                            <input id="add-image-{{$principal["category_id"]}}-{{ $principal["id"] }}" class="malfunction-upload-image" type="file" {{ isset($malfunction['malfunction_type'][$principal["category_id"]][$principal["id"]]) && in_array($malfunction['malfunction_type'][$principal["category_id"]][$principal["id"]], ['S', 'B']) ? '' : 'disabled' }}>
                                                        </label>
                                                    </div>
                                                    <div class='cb'></div>
                                                </div>
                            ...
                            <div class="col-md-12 malfunction-repaired malfunction-other hidden">
                                <div>
                                    ...
                                    <h4 class="malfunction-heading">
                                        <input type="checkbox" name="data[malfunction-culinary][checked]" {{$checked ? "checked" : ""}} id="malfunction-culinary-check" />
                                        5. <span class="{{ $checked ? "" : "hidden"}} malfunction-culinary-heading"> {{__('Culinary')}} </span> </h4>
                                </div>
                                <textarea name="data[malfunction-culinary][text]" id="malfunction-culinary-textarea" class="{{ $checked ?  "summernote-textarea" : "hidden" }}">
                            ...
                            <div class="col-md-12 malfunction-list malfunction-other hidden">
                                ...
                                <textarea name="data[malfunction-list]" id="malfunction-list-textarea" class="summernote-textarea">
                            ...
                            <div class="col-md-12 malfunction-summary malfunction-other hidden">
                                <div><h4 class="malfunction-heading">7. {{__('Summary & Recommendations')}}</h4></div>
                                <textarea name="data[malfunction-summary]" id="malfunction-summary-textarea" class="summernote-textarea">
                            ...
                            <div class="col-md-12 malfunction-uploaded-documents malfunction-other hidden">
                                ...
                                <div id="malfunction-uploaded-documents">
                                    @isset($malfunction['malfunction-uploads'])
                                        ...
                                        <div class="malfunction-uploads__item">
                                            ...
                                            <input type="hidden" name="data[malfunction-uploads][]" value="{{$upload}}"/>
                                        </div>
                                        @endforeach
                                    @endisset
                                    </div>
                                ...
                                <div style='display: flex; justify-content: center; align-items: center; margin-top: 10px;'>
                                    <a href="javascript:void(0);" id="send_to_admin" class="btn-blue">{{__('Send to admin')}}</a>
                                </div>
                            </form>
                        </div>
                    </div>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        var Lang = {
            ...
            "Message sent succusfully" : "{{__('Message sent succusfully')}}",
